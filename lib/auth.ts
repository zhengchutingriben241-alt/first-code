...
export function createAuthCookieHeader() {
  ...
  return `${COOKIE_NAME}=${encodeURIComponent(token)}; HttpOnly; Path=/; Max-Age=${MAX_AGE}; SameSite=Lax; ${secureFlag}`
}

export function clearAuthCookieHeader() {
  const secureFlag = process.env.NODE_ENV === 'production' ? 'Secure; ' : ''
  return `${COOKIE_NAME}=deleted; HttpOnly; Path=/; Expires=${new Date(0).toUTCString()}; Max-Age=0; Path=/; SameSite=Lax; ${secureFlag}`
}
